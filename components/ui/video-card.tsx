...
export function VideoCard({ project }: { project: Project }) {
 ...
 className="group relative rounded-3xl overflow-hidden aspect-[4/5] glass border border-white/10 h-full"
 >
 <div
 className="pointer-events-none absolute -inset-px opacity-0 transition duration-300 z-30"
 style={{
 opacity,
 background: `radial-gradient(600px circle at ${position.x}px ${position.y}px, rgba(125,145,154,.15), transparent 40%)`,
 }}
 />

 <div className="absolute inset-0 bg-zinc-900 z-0">
 {project.video ? (
 <video src={project.video} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-60 grayscale group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" />
 ) : (
 <img src={project.image} alt={project.title} className="w-full h-full object-cover opacity-60 grayscale group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" />
 )}
 </div>
 ...
 <div className="w-24 h-24 rounded-full glass flex items-center justify-center bg-white/5 backdrop-blur-xl border border-white/20 text-white shadow-[0_0_40px_rgba(255,255,255,0.15)]">
 ...
 <div>
 <span className="text-slate-light text-xs font-bold tracking-wider uppercase mb-3 block">
 {project.category}
 </span>
