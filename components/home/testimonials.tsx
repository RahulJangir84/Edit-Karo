...
const TESTIMONIALS = [
 ...
 avatar: "AM",
 color: "from-slate-medium to-slate-light",
 },
 {
 name: "Priya Sharma",
 ...
 avatar: "PS",
 color: "from-slate-dark to-slate-medium",
 },
 {
 name: "Ravi Kapoor",
 ...
 avatar: "RK",
 color: "from-slate-light to-slate-dark",
 },
];
...
export default function Testimonials() {
 ...
 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[40vw] bg-slate-dark/10 rounded-full blur-[120px] pointer-events-none" />
 ...
 <button onClick={prev} className="w-12 h-12 rounded-full glass border border-white/10 flex items-center justify-center text-white hover:border-slate-light/50 hover:text-slate-light transition-all duration-300 ">
 <ChevronLeft className="w-5 h-5" />
 </button>
 <div className="flex gap-2">
 {TESTIMONIALS.map((_, i) => (
 <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === current ? "bg-slate-light w-6" : "bg-white/20"}`} />
 ))}
 </div>
 <button onClick={next} className="w-12 h-12 rounded-full glass border border-white/10 flex items-center justify-center text-white hover:border-slate-light/50 hover:text-slate-light transition-all duration-300 ">
