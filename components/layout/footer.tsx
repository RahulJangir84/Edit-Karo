  ...
  <span className="bg-gradient-to-r from-white via-neutral-300 to-neutral-500 bg-clip-text text-transparent">
    Stop The Scroll?
        ...
        <nav className="flex gap-6 flex-wrap justify-center">
          {FOOTER_LINKS.map((l) => (
            <Link key={l.label} href={l.href} className="hover:text-white transition-colors cursor-none">
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex gap-4">
          {SOCIALS.map(({ icon: Icon, href, label }) => (
            <a
              ...
              aria-label={label}
              className="w-10 h-10 rounded-full glass border border-white/10 flex items-center justify-center text-gray-400 hover:text-accent-cyan hover:border-accent-cyan/50 transition-all duration-300 cursor-none"
            >
